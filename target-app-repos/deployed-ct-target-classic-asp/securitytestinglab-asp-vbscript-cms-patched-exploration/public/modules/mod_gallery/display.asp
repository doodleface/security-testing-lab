...
function getContent()
	...
	Call debug("mod_gallery.display: starting mod_gallery module....")
	...
	if not settings.exists("gallery_folder") then
		...
		Call debug("mod_gallery.display: content to display: " & Server.HtmlEncode(gallery_folder) )
		...
		if not isObject(fileDict) or fileDict is nothing then
			...
			Call debugInfo("mod_gallery.display: number of images " & fileDict.count)
			if fileDict.count > 0 then
				dim filename, path, name, ext, url, thmb, lnk, page, num_pages
				content.add "<div class=""photo-gallery"">" & vbCrLf
				dim i : i = 0
				num_pages = 0
				Call debugInfo("mod_gallery.display: the current page is '" & current_page & "'.")
				for each fileName in fileDict
					...
					Call debugInfo("mod_gallery.display: found file '" & name & "' url string '" & url & "' thumb is '" & thmb & "' and page is '" & page & "'")
