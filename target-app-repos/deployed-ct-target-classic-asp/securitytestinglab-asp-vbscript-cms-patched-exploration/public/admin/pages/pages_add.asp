...
function contentAdd()
	...
	if myForm.wasSubmitted() = true then
		Call debugInfo("form was submitted, storing to the session.")
		...
		if formErrors.count > 0 then
			...
			for i = 0 to max - 1
				if len(path(i)) > 0 then
					...
					if fs.FolderExists(sitepath & path_write) = FALSE then
						...
						Call trace("folder '" & sitepath & path_write & "' exists.")
					end if
					cfolder = cfolder & "../"
			...
			if id = "" then
				Call debugError("There was an error creating ")
				...
				Call debug("new " & strContent & " id is '" & id & "'")
