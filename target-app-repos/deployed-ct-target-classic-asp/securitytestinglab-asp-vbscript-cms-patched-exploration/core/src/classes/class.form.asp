...
class WebForm
	...
	private sub class_Initialize()
      ...
      '
			err.clear
			Call debugWarning("class.form.init: form upload functionality is disabled (SoftArtisans.FileUp module not found)")
		...
		if len(m_Request.Form("form_name")) > 0 then
			Call debugInfo("class.form.init: form was submitted with " & m_Request.Form.count & " fields.")
			Call debugInfo("class.form.init: setting form name to submitted form name.... ")
	...
	private function getDefaultPath()
		...
		Call trace("class.form.getDefaultPath: '" & currentFilePath & "'")
	...
	public function setName(byval strName)
		...
		if isEmpty(strName) or isNull(strName) or strName="" then
			Call debugWarning("class.form.setName: '" & strName & "'")
			exit function
		end if
		Call debugInfo("class.form.setName: '" & strName & "'")
	...
	public function setAction(strAction)
		Call debugInfo("class.form.setAction: '" & strAction & "'")
		m_Action = "" & strAction
	end function
	...
	public function setMethod(strMethod)
		Call debugInfo("class.form.setName: '" & strMethod & "'")
		m_Method = strMethod
	end function
	...
	public property let UploadPath(strPath)
		if instr(strPath,":\") > 0 then
			Call debugError("class.form.uploadPath: '" & strPath & "' is Absolute, and is not allowed using this script!")
			m_UploadPath = getDefaultPath()
			Call debugError("class.form.uploadPath: using '" & m_UploadPath & "' (default) instead.")
		elseif (instr(strPath, "\") = 1) or (instr(strPath, "/") = 1) then
			Call trace("class.form.uploadPath: '" & strPath & "' is relative to the site root.")
			m_UploadPath = strPath
		else
			 'removes the filename from the path
			Call trace("class.form.uploadPath: '" & strPath & "' is realtive to this file's location (" & getDefaultPath() & ")")
		...
		Call debugInfo("class.form.uploadPath: '" & m_UploadPath & "'")
	end property
	...
	public function getValue(strItemName)
		'on error resume next
		Call trace("class.form.getValue: raw value for " & strItemName & " is ")
		on error resume next
		Call trace("class.form.getValue: '" & m_Request.Form("" & strItemName) & "'")
		TrapError
		...
		if err.number <> 0 then
			...
			Call trace("class.form.getValue: " & strFormWidgetName & "= '" & m_Request.Form("" & strItemName) & "'")
		else
			getValue = m_Request.Form("" & strItemName).ShortFilename
			Call trace("class.form.getValue: " & strItemName & "= '" & m_Request.Form("" & strItemName).ShortFilename & "' (filename)")
		end if
	end function
	...
	public function getContents()
		...
		if isNull(m_Name) or m_Name = "" then
			...
			setName("form" & rand)
			Call debug("class.form.getContents: a name for this form was never specified, applying a random name '" & m_Name & "'")
		end if
	...
	public function CreateNew(aName, anAction)
		Call debugWarning("DEPRICATED: class.form.createNew is marked for removal in the upcomming release!")
		setName(aName)
		setAction(anAction)
	...
	function endNoteSection()
		dim i 'counter
		Call debug("class.form.endNoteSection: There are " & m_dictNotes.count & " notes.")
		for i=0 to m_dictNotes.count-1
			Call debug("FORM NOTE " & i+1 & ": '" & m_dictNotes.keys()(i) & "' '" & m_dictNotes.items()(i) & "'")
	...
	function addFormSubmission(sAlignment,sSubmitText, sResetText, sClass, sDescriptionText)
		Call debug("FORM SUBMIT BUTTON: text='" & sSubmitText & "' reset='" & sResetText & "' description='" & sDescriptionText & "'")
	...
	function addFormButton(sType,sButtonText,sClass,sCustomTags,sDescriptionText)
		...
		Call debug("FORM BUTTON: text='" & sButtonText & "' description='" & sDescriptionText & "'")
	...
	function addFormInput(sRequired, sName, sId,  sFieldType, sClass, sDefaultVal, sCustomTags, sMessage)
		...
		Call trace("")
		Call trace(UCASE(sFieldType) & " FIELD: required='" & isRequired & "' name='" & sGroup & "' id='" & sId & "' class='" & sClass & "' value='" & Server.HtmlEncode("" & sDefaultVal) & "' tags='" & sCustomTags & "'")
		select case sFieldType
			...
			case "checkbox","radio"
				...
				Call trace("is this button grouped? " & isGrouped)
				Call trace(sFieldType & " is selected by default? " & isChecked)
			...
			case "image"
				Call debugError("class.form.addFormInput: Please do not use form inputs with type='image' because they are not accessible.")
			case else
				Call debugError("class.form.addFormInput: There is no such form input for type '" & sFieldType & "'")
		end Select
	end function
	...
	function addFormSelect(sRequired, sName, sId, sClass, sCustomTags)
		...
		Call debug("class.form.addFormSelect: '" & sId & "'")
		dim a : set a = New FastString
		Call trace("class.form.addFormSelect: '" & sRequired & "' name='" & sName & "' id='" & sId & "' class='" & sClass & "' tags='" & sCustomTags & "'.")
	...
	function addFormOption(selectId, sValue, sDisplay, sSelected)
		...
		if currentSelectGroup <> "" then
			if currentSelectGroup <> selectId then
				Call debugError("class.form.addFormOption: SELECT group specified for this OPTION (" & selectId & ") does not coinside with the actual parent tag that is open right now, using the default that is open (" & currentSelectGroup & ")")
			end if
			'if isMultiSelect = true then sName = "name=""" & currentSelectGroup & """ "

			Call trace("class.form.addFormOption: OPTION FOR SELECT GROUP(" & currentSelectGroup & "): '" & sSelected & "' value='" & sValue & "' groupId='" & currentSelectGroup & "' display='" & sDisplay & ".")
			...
			Call debugError("class.form.addFormOption: there is currently no Select Group for which to add this option!")
			Call debugError("class.form.addFormOption: use addFormSelect() to initialize a select group first")
		end if
	end function

	function endFormSelect(byval sMessage)
		Call debug("class.form.endFormSelect: '" & currentSelectGroup & "'")
		if isNull(currentSelectGroup) or (currentSelectGroup = "") then
			Call debugError("there is no Select Group to close")
			Call debugError("Please use addFormSelect() to initialize a select group")
		else
			...
			if m_dictErrors.exists("" & currentSelectGroup) then
				Call trace("errors were found in this Select Group")
	...
	function addFormGroup(sFieldType,sRequired,sDisplayName,sId)
		...
		dim a : set a = New FastString
		Call trace("")
		Call trace( UCASE(sFieldType) & " GROUP: '" & sRequired & "' displayName='" & sDisplayName & "' id='" & sId & "'.")
		isGrouped = true
		...
		Call trace("class.form.addFormGroup: '" & currentGroupId & "'")
	end function
	...
	function addFormRadioButton(sid, sValue, sLabel, sClass, sChecked)
		...
		Call trace("class.form.addFormRadioButton: group='" & sGroup & "' id='" & sid & "' value='" & sValue & "'  class='" & sClass & "' checked='" & sChecked & "'.")
	...
	private function validateFormField(byval sRequired,byval sName,byval sid,byval sFieldType,byval sClass)
		...
		if wasSubmitted() then
			Call trace("class.form.validateFormField: '" & sid & "'")
		'only check field values if form was submitted
			...
			'end if
			case else
				Call debugWarning("class.form.validateFormField: field of type " & sFieldType & " is not currently supported for validation.")
			end select
		else
			Call trace("class.form.validateFormField: form not submitted nothing to validate")
		end if
		if validateFormField then
			Call debugInfo("class.form.validateFormField: '" & sid & "' values are valid. ")
		else
			Call debugInfo("class.form.validateFormField: '" & sid & "' values are invalid. ")
		end if
	end function
	...
	private function requireValue(fldName, id)
		requireValue = true
		Call debug("its value is '" & m_Request.Form("" & id) & "'")
	...
	private function requireLength(fldName, id, classString)
		...
		next
		Call debug("class.form.requireLength: " & fldName & " requires min length of " & min & " and max length of " & max & ".")
	...
	private function checkCreditCard(fldName, id)
		...
		Call debugError("class.form.checkCreditCard: validation is not yet implemented for ASP it does however work in javascript, there is no guarantee that the end user will have javascript on so please refrain from using this unless you want to implement the error checking.")
	'	dim val : val = m_Request.Form("" & id)
	'	dim ctype
	'	if val <> "" then
	...
	private function checkZip(fldName, id)
		...
		if len(val) > 0 then
			...
			Call trace("class.form.checkZip: this zip code is of length" & len(val))
			if not isnumeric(val) then
				addFormError id, "The " & fldName & " field must be numeric."
				checkZip = FALSE
			elseif not anInteger.test(val) then
			...
			Call trace("class.form.checkZip: Zip Code was empty")
		end if
	end function
	...
	public function hasErrors()
		Call trace("class.form.hasErrors(): m_dictErrors.count is " & m_dictErrors.count)
	...
	private sub addFormError(id, sMessage)
		Call debug("class.form.addFormError: field:" & id & "; error:" & sMessage)
	...
	private function valueFill(sFieldType,id,defaultVal,defaultOption)
		Call trace("class.form.valueFill(" & sFieldType & "," & id & "," & Server.HtmlEncode("" & defaultVal) & "," & defaultOption & ")")
	...
	'===START DECISION TREE ====================================
		select case sFieldType
			case "text","password","file","hidden"
				...
				if wasSubmitted() then
					Call trace("class.form.valueFill: form was submitted, " & id & " gets value " & formVal)
					valueFill = "value=""" & formVal & """"
				elseif wasStoredToSession(m_Name) and (sessVal <> "") then
					Call trace("class.form.valueFill: session has non-null contents for " & m_Name & ":" & id & ": gets value " & sessVal)
					valueFill = "value=""" & sessVal & """"
				else 'just use the default variables passed in
					Call trace("class.form.valueFill: no valid stored values for '" & id & "' so it gets default value '" & defaultVal & "'")
			...
			case "textarea"
				...
				if wasSubmitted() then
					Call trace("class.form.valueFill: form was submitted, " & id & " gets value " & Server.HtmlEncode("" & formVal))
					valueFill = formVal
				elseif wasStoredToSession(m_Name) then
					Call trace("class.form.valueFill: session has non-null contents for " & m_Name & ":" & id & " and thus gets value " & Server.HtmlEncode("" & sessVal))
					valueFill = sessVal
				else
					Call trace("class.form.valueFill: no valid stored values for '" & id & "' so it gets default value '" & Server.HtmlEncode("" & defaultVal) & "'")
			...
			case "radio", "checkbox"
				...
				if wasSubmitted() then
					if (formVal = defaultVal) then
						...
						Call trace("class.form.valueFill: submitted value matches default value, so this " & sFieldType & " gets checked")
						valueFill = valueFill & " checked=""checked"""
					end if
				elseif wasStoredToSession(m_Name) and (sessVal <> "") then
						Call trace("class.form.valueFill: session has non-null contents for '" & id & "' :" & sessVal)
						if  (sessVal = defaultVal) then
						Call trace("class.form.valueFill: the session value matches this " & sFieldType & "'s default value so it gets checked")
					...
					if instr(defaultOption,"checked") then
						Call trace("class.form.valueFill: " & sFieldType & " with id '" & id & "' has default checked options and no session values were found so it is thus  enabled")
						valueFill = valueFill & " checked=""checked"""
					else
						Call trace("class.form.valueFill: " & sFieldType & " with id '" & id & "' has no default checked option.")
					end if
				end if
			case "option"
				...
				if wasSubmitted() and (formVal <> "") then
					if (formVal = defaultVal) then
						...
						Call trace("class.form.valueFill: " & "submitted value matches default value, so this " & sFieldType & " gets selected")
				...
				elseif wasStoredToSession(m_Name) and (sessVal <> "") then
					...
					if instr(defaultOption,"selected") then
						Call trace("class.form.valueFill: " & sFieldType & " with id '" & id & "' has default selected option, and no session values were found so it is thus enabled")
						valueFill = valueFill & SELECTED
					else
						Call trace("class.form.valueFill: " & sFieldType & " with id '" & id & "' has no default checked option.")
					end if
			...
			case else
				Call debugError("class.form.valueFill: " & " type '" & sFieldType & "' handling is not yet implemented!")
		end select
	'===END DECISION TREE====================================
		Call debug("class.form.valueFill:  '" & id & "' => '" & Server.HtmlEncode("" & valueFill) & "'")
	end function
	...
	public function wasSubmitted()
		...
		Call trace("class.form.wasSubmitted: this.form.name = '" & m_Name & "'")
		Call trace("class.form.wasSubmitted: m_Request.Form.name ='" & m_Request.Form("form_name") & "'")
		'trace("class.form.wasSubmitted: Request.Form.name ='" & Request.Form("form_name") & "'")
		if m_Name = "" then
			Call debugWarning("class.form.wasSubmitted:  there was no form name specified, its generally a better idea to use setName() before any other form manipulations!")
		...
		elseif m_Request.Form("form_name") = m_Name then
			...
			if (not isSubmissionChecked) then
				Call debugInfo("class.form.wasSubmitted: '" & m_Name & "' was submitted by " & Request.ServerVariables("REQUEST_METHOD") & " method and contains " & m_Request.Form.count & " fields.")
			end if
			submitted = (m_Request.Form("form_name") = m_Name)
		else
			Call trace("class.form.wasSubmitted:  form " & m_Name & " has not yet been submitted.")
		end if
	...
	public function storeFormToSession()
		Call debug("class.form.storeFormToSession: storing form submission to session")
		if m_dictErrors.Count <> 0 then
			Call debugError("class.form.storeFormToSession: errors were found in the user's sumbission.")
		end if
		dim fld
		if isNull(m_Name) or m_Name = "" then
			m_Name = m_Request.Form("form_name")
			Call debug("class.form.storeForemToSession:  no form name was specified. using name '" & m_Name & "' found on the request object!")
		end if
		for each fld in m_Request.Form
			Call trace("class.form.storeFormToSession: storing " & fld & " to session with value '" & m_Request.Form(fld) & "'")
	...
	private function wasStoredToSession(strName)
		...
		'see if specified form name was used to send data to this form using Session variables
			if (not isSessionChecked) then
				Call debugInfo("class.form.wasStoredToSession: form '" & strName & "' was redirected from " & Request.ServerVariables("HTTP_REFERER") & " and form fields are stored as Session variables")
			end if
			sessionSaved = true
		end if
	...
	private function clearFormFromSession(strName)
		...
		if isNull(strName) Or strName = "" then
			Call debugError("form.class.clearFormFromSession: you must specify a form name to remove from the session")
		else
			dim key, id
			...
			for each key in Session.Contents
				...
				i = i+1
			next
			Call debug("form.class.clearFormFromSession: removing session keys for form '" & strName & "'....")
			for i=0 to ubound(keys)-1
				if instr(keys(i),":") > 0 then
					if split(keys(i),":")(0) = strName then
						Call trace("form.class.clearFormFromSession: removing " & keys(i) & " from session")
						Session.Contents.Remove(keys(i))
					end if
				else
					Call trace("form.class.clearFormFromSession: leaving " & keys(i))
				end if

			next
			for each key in Session.Contents
				Call trace("form.class.clearFormFromSession: " & key & " is still here.")
			next
		end if
	...
	public function fileUp(id, path, fileFormats, fileSizeLimit)
		...
		if id = "" or isNull(id) then
			Call debugError("form.class.fileUp: no form field id was specified!")
			exit function
		end if
		...
		if m_Request.Form(id).isEmpty then
			Call debug("form.class.fileUp: no file in form field '" & id & "' to upload.")
			if err.number <> 0 then
				...
				Call debugError("form.class.fileUp: Important Note! File Upload is not enabled therefore field with id '" & id & "' and value '" & lcase(trim(m_Request.Form(id))) & "' will not be uploaded.")
				err.clear
				exit function
			end if
		exit function
		...
		Call debug("form.class.fileUp: uploading file in form field '" & id & "' ...")
		...
		if not isNull(fileSizeLimit) then
			if CLng(fileSizeLimit) > 0 then
				...
				Call debugError("form.class.fileUp:  fileSizeLimit must be a valid long integer")
			end if
			if err.number <> 0 then
				Call debugError("form.class.fileUp:  fileSizeLimit '" & fileSizeLimit & "' is not a valid Long")
				err.clear
			end if
		end if
		...
		if path = "" or isNull(path) then
			Call trace("form.class.fileUp: no path specified; default UploadPath applied: " & m_UploadPath)
		else
			UploadPath = path
		end if
		...
		if err.number <> 0 then
			Call debugError("form.class.fileUp:  error produced during attempt to set maxbytes to '" & maxBytes & "'")
			err.clear
		end if
		...
		if not m_Request.Form(id).IsEmpty then
			Call debugInfo(" saving file... '" & globals("SITE_PATH") & m_UploadPath & m_Request.Form(id).ShortFilename & "'")
			m_Request.Form(id).SaveAs ( globals("SITE_PATH") & m_UploadPath & m_Request.Form(id).ShortFilename)
			if err.number <> 0 then
				addFormError "" & id, "" & err.description
			end if
			Call trace("form.class.fileUp: saving file '" & m_Request.Form(id).ShortFilename & "' to " & m_Request.Path)
			fileUp = m_UploadPath & m_Request.Form(id).ShortFilename
			if err.number <> 0 then
				Call debugError("form.class.fileUp:  error produced during attempt to test ShortFilename property")
				err.clear
			end if
		end if
		if err.number <> 0 then
			Call debugError("form.class.fileUp:  error produced during attempt to test IsEmpty property")
			err.clear
		end if

		if err.number <> 0 then
			Call debugError("form.class.fileUp:  error produced during attempt to upload a file from form field '" & id & "'")
			err.clear
		end if

		erase contentFormatArray
		set contentFormat = nothing
		Call debug("form.class.fileUp: file uploaded successfully to '" & m_Request.Form(id).Path & "'")
	end function
	...
	function getFolderListOptions(byref theForm, byval currentFolder,byval selectedFolder,byval optionId)
		...
		Call debug("")
		Call debug("")
		Call debug("form.getFolderListOption('" & currentFolder & "','" & selectedFolder & "')")
