...
if len(myQueryString) > 0 then
	if instr(myQueryString, ";") > 1 then
		...
		Call debugInfo("404.asp: Server error is '" & myErr & "'")
		if instr(myURL,"?") then
	...
	if len(myCss) > 0 then
		Call debugInfo("404.asp: CSS style override is '" & myCss & "'")
	end if
end if
Call trace("404.asp: Query string is '" & myQueryString & "'")
Call trace("404.asp: Requested page is '" & myURL & "'")
if len(myCss) > 0 then
	page.overrideStyle(myCss)
end if
...
if not db.exists() then
	...
	Call debugError("404.asp: A homepage was not found in the database for your site. We are going to populate with some error text, for testing/display.")
else
	Call trace("404.asp: site enabled? " & settings.getItem("Enabled Site"))
	Call trace("404.asp: Processed URL Request is '" & myURL & "'")
	if settings.getItem("Enabled Site") <> "1" then
		...
		if not setFile OR page.exists = false then
			Call trace("404.asp: File or DB contents matching '" & myURL & "' does not exist!")
			Server.Execute("core/include/utils/error404.asp")
		else
			Session.Contents("Page") = myURL
			Call trace("404.asp: the page '" & myURL & "' was found in the database.")
		end if
