...
function createTable(byVal strName,byVal strPath)
	if strName = ""  or strPath = "" then
		...
		Call debugError("when calling this page, you must pass a name and path-to-sql-create-file on the querystring")
		exit function
	end if
	if not db.Init(strName) is nothing then
		createTable = ErrorMessage("Cannot install table '" & strName & "' because it already exists!")
		Call debugError("the table '" & strName & "' already exists in database '" & dbPath & "'")
		exit function
	end if
	...
	if installFile.fileExists() = false then
		...
		Call debugError("please specify an existing file for the sql instructions, '" & strPath & "' does not exist.")
		exit function
	end if
	...
	do until ts.atendOfStream
		...
		if len(sql) > 0 and not instr(sql, "--") = 1 then
			Call debug("instrrev(sql,';') = " & instrrev(sql,";") & "")
			Call debug("len(sql) = " & len(sql) & "")
			if instrrev(sql, ";")>0 then
				Call debug("executing SQL: '" & sql & "'...")
				if not db.execute(sql) then
					...
					Call debugError("there was an error in creating table '" & strName & "' with SQL (" & sql & ")")
