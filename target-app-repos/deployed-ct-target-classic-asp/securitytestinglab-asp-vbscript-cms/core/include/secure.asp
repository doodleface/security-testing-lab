...
Call trace("Secure File:  user.roleLevel=" & user.getRole() & " page.secureitylevel=" & page.getSecurityLevel())
requested_url = ""
if user.getRole() >= page.getSecurityLevel() then
	Call debug("secure.asp: user has sufficient rights to access secured file.")
	if (user.isLoggedIn()) then
		Call debugInfo("secure.asp: user is logged in.")
	else
