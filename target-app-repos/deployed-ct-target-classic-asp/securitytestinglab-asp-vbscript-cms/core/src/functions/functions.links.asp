...
function createNavLink(byval url,byval content,byval title,byval liclass)
	...
	Call debug("url:" & url & " request:" & Request.ServerVariables("url") & " page.filename:" & page.getFileName() & " page.filepath:" & page.getfilepath)
...
'TODO: fix this function to work with real relative paths (eg ../../file.asp)
	if InStr(relativePath,globals("SITEURL")) > 0 then
		Call debugWarning("urlRelToAbs(): path '" & relativePath & "' is already absolute")
	elseif InStr(relativePath,"http") = 1 then
		Call debugWarning("urlRelToAbs(): path '" & relativePath & "' is already absolute")
	elseif instr(relativePath,"./") > 0 then
		Call debugError("urlRelToAbs(): relative paths using './' or '../' have not yet been implemented!!!!")
	else
	...
	if instrrev(lcase(relativePath),"/default.asp") = len(relativePath) - 11 then
		relativePath = replace(lcase(relativePath), "/default.asp", "/")
	end if
...
function adminEditLink(linkUrl, linkText, linkHoverText)
	if user.isAdministrator then
		Call trace("adminEditLink: user has active admin session... adding link '" & linkText & "'")
		dim url : url = replace(linkUrl, "\", "/")
		if InStr(url, globals("ADMINURL")) > 0 then
			Call trace("adminEditLink: [" & url & "] an absolute link was provided, no modifications applied")
			'do nothing to modify the link
		elseif InStr(url,"http://") > 0 then
			Call debugError("adminEditLink:  [" & url & "] absolute link provided does not link to current admin area [" & globals("ADMINURL") & "]")
		else
