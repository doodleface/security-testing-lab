...
class ClsDatabase
	...
	public function ConnectOpen(byVal strDBName)
		...
		Call debug("class.db.connectOpen:  opening " & strDBName)
		...
		if err.number <> 0 then
			...
			Call debugError("class.db.connectOpen: The database " & strDBName & " could not be found.")
		else
			Call debug("class.db.ConnectOpen: The database  " & strDBName & " was opened successfully.")
			m_bConnected = True
			m_bExists = True
	...
	public function BuildList(byRef refArrDic(), byVal strTable, byVal strCondition)
		...
		if strCondition <> "" then
			if not instr(trim(lcase(strCondition)),"order by") = 1 then
				strSQL = strSQL & " where "
			end if
	...
	public function Execute(byVal strSQL)
		Call trace("class.db.execute: SQL='" & Server.HtmlEncode(strSQL) & "'")
		if not m_bConnected then
			Call debugError("class.db.execute: Cannot execute '" & Server.HtmlEncode(strSQL) & " because you are not connected to a database.  Use class.db.ConnectOpen() first.")
			Execute = false
			exit function
		end if
		...
		on error resume next
			...
			if Err.number <> 0 then
				if instr(lcase(strSQL, "update")) = 1 then
					m_isWritable = false
				end if
				bOK = false
	...
	public function Space(byVal strTable, byVal strField, byVal strCondition)
		...
		if Len(strField) > 0 then
			strSQL = "select sum(" & strField & ") as totalsize from " & strTable
		end if
		if Len(strCondition) > 0 then
			strSQL = strSQL & " where " & strCondition
		end if
	...
	public function BuildPage(byRef refArrDic(), byVal strTable, byVal strCondition, byVal nPageIndex, byVal nMsgsPerPage, byRef nPageCount)
		...
		if eval(nPageIndex) > eval(nPageCount) then
			nPageIndex = nPageCount
		end if
		if not rs.Eof then
			rs.Move nPageIndex * nMsgsPerPage
		end if
	...
	'* Start a database transaction. A transaction must subsequently be ended
	'* in order to commit (or rollback, on error) the db call(s).
	'* @return false if a transaction is already open.
	'*
	public function StartTransaction()
		StartTransaction = false
		if m_inTransaction = true then
			Call debugError("class.db.StartTransaction: an existing transaction was already started, please commit the transaction using EndTransaction() before attempting to start another transaction. ")
			exit function
		end if
	...
	public function EndTransaction()
		...
		if m_inTransaction = false then
			Call debugError("class.db.EndTransaction: there is no current transaction to end. ")
			exit function
		end if
		if m_hasErrors = true then
			Call debugInfo("class.db.EndTransaction: the execution had errors! rolling back transaction...")
			m_objConnection.RollbackTrans
			Call debugInfo("class.db.EndTransaction: ...rollback complete.")
			m_hasErrors = false
		else
			Call debugInfo("class.db.EndTransaction: committing transaction...")
			m_objConnection.CommitTrans
			Call debugInfo("class.db.EndTransaction: ...commit complete.")
			EndTransaction = true
		end if
		m_inTransaction = false
	end function
	...
	public function rsOpen(record_set, byVal str_sql, intCursorType, intLockType)
		...
		on error resume next
			err.clear
			Call trace("class.db.rsOpen '" & str_sql & "' CursorType:" & intCursorType & " LockType:" & intLockType)
			...
			if (err.number <> 0) or (record_set.state = 0) then
				rsOpen = false
				Call debugWarning("class.db.rsOpen: no recordset returned for query '" & str_sql & "'")
			end if
			trapDBError
			incrementDBCalls
		on error goto 0
	...
	private function trapDBError()
		...
		if isObject(m_objConnection) Then
			if m_objConnection.Errors.Count > 0 Then
				trapDBError = true
				m_hasErrors = true
				Call trace("The server execution encountered the following database errors:")
