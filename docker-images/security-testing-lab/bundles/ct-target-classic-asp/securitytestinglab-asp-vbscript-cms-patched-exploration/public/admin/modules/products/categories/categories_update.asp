...
function contentUpdate()
	...
	if myForm.wasSubmitted() = true then
		...
		if formErrors.count = 0 then
			...
			Call debug("updating database content for category '" & myForm.getValue("Category") & "'")
		  ...
		  Call trace("admin." & strContentPL & ".update: start file upload...")
		  dim duration : duration = timer
		  dim fileName1 : fileName1 = myForm.fileUp("Image1", strUploadPath, strFileFormats, maxFileSize)
		  Call trace("admin." & strContentPL & ".update: ...end file upload,  duration:" & timer - duration)
			strError = ""
		else
