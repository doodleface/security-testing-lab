...
class SiteFile
	...
	public sub Class_Initialize()
		if globals("SITE_PATH") = "" then
			Call debugError("class.file.init: global variable 'SITE_PATH' is required but was not found")
		else
			Call trace("class.file.init: global variable 'SITE_PATH' is '" & globals("SITE_PATH") & "'")
		end if
		fUrl = ""
		fPath = ""
		fVirtualPath = ""
	...
	public property let Path(strPath)
		...
		if isEmpty(strPath) or isNull(strPath) or (strPath = "") then
			Call debugError("class.file.path: must provide a non-empty, non-null string to set the path.")
		else
			fPath = strPath
			Call trace("class.file.path: file name string is '" & strPath & "'")
			if instr(fPath, globals("SITEURL")) > 0 then
				...
				Call trace("class.file.path: the file is on this domain... converted Path to '" & fPath & "'")
			end if
			if instr(fPath, "http") = 1 then
				' Assume that a file exists if its not on this domain.
				fExists = true
				Call trace("class.file.path: the file '" & fPath & "'is not on this domain, and we assume it exists")
			else
				...
				fPath = replace(fPath, "/", "\")
				fPath = replace(fPath, "\\", "\")
				Call trace("class.file.path: getting absoulte path for '" & fPath & "'...")
				if left(fPath, 1) = "\" then
					Call trace("class.file.path: path specified was a virtual path based off of the site root")
					...
					if instr(fPath, globals("SITE_PATH")) = 1 then
						Call trace("class.file.path: path specified was absolute path inside this site.")
						fAbsolutePath = fPath
						fPath = right(fPath, len(globals("SITE_PATH")))
					else
						Call trace("class.file.path: path specified was a virtual path based off of the current folder")
						fAbsolutePath = Server.MapPath(fPath)
					end if
				end if
				Call trace("class.file.path: file absolute path is '" & fAbsolutePath & "'")
				...
				if inStr(fAbsolutePath, globals("SITE_PATH")) > 0 then
					...
					Call trace("class.file.path: file virtual path is '" & fVirtualPath & "'")
					Call trace("class.file.path: file complete url is '" & fUrl & "'")
				end if
		...
		if fExists then
			Call debugInfo("class.file.path: file at path '" & fPath & "' exists")
		else
			Call debugWarning("class.file.path: file at path '" & fPath & "' does not exist")
		end if
	end property
	...
	public function run()
		if not fileExists() = true then
			Call debugError("class.file.run: module handler '" & fVirtualPath & "' does not exist!")
		else
			Call trace("class.file.run: executing module: '" & fVirtualPath & "'")
			on error resume next
			Server.Execute(fVirtualPath)
			if err.number <> 0 then
				Call debugError("class.file.run: error executing '" & fVirtualPath & "'")
				Call debugError("class.file.run: " & err.number & " - " & err.description)
				err.clear
			end if
		end if
	end function
	...
	public function copy_file(byval strfilesource, byval strfiledestination, byval stroverwrite)
		...
		for i = 0 to max - 1
			if len(path(i)) > 0 then
				...
				if fs.FolderExists(globals("SITE_PATH") & path_write) = FALSE then
					Call debugInfo("Folder '" & globals("SITE_PATH") & path_write & "' created!")
						set f = fs.CreateFolder(globals("SITE_PATH") & path_write)
						set f = nothing
				else
					Trace("Folder exist!")
				end if
				Call debugInfo(globals("SITE_PATH") & path_write & "<br>")
			end if
		next
		on error resume next
		...
		copy_file = TRUE
		Call debugInfo("Copied '" & strfilesource & "' to '" & globals("SITE_PATH") & path_write & "\" & path(max) & "'")
	end function
	...
	public function write_file(byval strfilename, byval strcontent, byval collisionResolution)
		...
		write_file = TRUE
		Call debugInfo("Created '" & strfilename & "'")
	end function
	...
	function getFiles(byval fileExtensions)
		...
		Call debug("class.file.getFilesInFolder: absolute folder path is " & AbsolutePath )
		on error resume next
				...
				if (len(ext) > 0) and (len(item.Name) > len(ext)) then
					if (instr(item.Name, ext) = len(item.name) - len(ext) + 1) then
						if not bIncludeExtensions then
							Call trace(" [ " & replace(item.Name, ext, "") & " => " & item.path & " ]")
							resultDict.add replace(item.Name, ext, ""), item.path
						else
							Call trace(" [ " & item.Name & " => " & item.path & " ]")
...
function getFilesInFolder(strPath, fileExtensions)
	...
	fileTypes = split(fileExtensions, ",")
	bIncludeExtensions = (ubound(fileTypes) <> 0)
	Call debug("class.file.getFilesInFolder: absolute folder path is " & file.absolutePath)
	on error resume next
