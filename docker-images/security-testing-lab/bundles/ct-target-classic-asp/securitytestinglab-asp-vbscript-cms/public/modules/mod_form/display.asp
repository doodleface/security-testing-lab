...
function getForm()
	...
	Call debug("mod_form.display: starting mod_form module....")
	if settings.exists("form_id") then
		id = settings("form_id")
		css = settings("external_css")
		Call debug("mod_form.display: query string was '" & Request.QueryString() & "'")
	end if
	...
	if rsForm.state = 0 then
		Call debugError("mod_form.display: there was an error getting form from database")
		exit function
	end if

	if rsForm.EOF or rsForm.BOF then
		Call debugError("mod_form.display: no form with id '" & id & "' exists in database")
		exit function
	end if

	Call debug("mod_form.display: creating form '" & id & "' from database...")
	...
	do until rsForm.EOF
		...
		Call trace("mod_form.display: fieldId=" & sId)
		Call trace("mod_form.display: fieldName=" & sName)
		Call trace("mod_form.display: fieldType=" & sFieldType)
		Call trace("mod_form.display: fieldValue=" & sDefaultVal)
		Call trace("mod_form.display: fieldValidation=" & sClass)
		Call trace("mod_form.display: fieldCustomTags='" & sCustomTags & "'")
		if sId = "form_name" then
			Call debugInfo("mod_form.display: initializing '" & sName & "' ...")
			...
			select case sFieldType
			 case "text","password","hidden","textarea"
				...
				case else
					Call debugError("there was an error processing field of type: " & sFieldType)
			end select
		end if
		rsForm.movenext
	loop
	...
	if myForm.wasSubmitted() and not myForm.hasErrors() then
		...
		Call trace("mod_form.display: executing form processing...")
...
function insertFormCSS(cssFile)
	...
	if instrrev(cssFile,".css")=len(cssFile)-3 then
		...
		Call debug("mod_form.display: external css '" & cssFile & "' not a valid css file!")
		cssFile = ""
	end if
	if cssFile = "" then
		%>
