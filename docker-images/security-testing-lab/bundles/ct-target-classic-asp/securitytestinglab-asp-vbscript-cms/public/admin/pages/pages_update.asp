...
function contentUpdate()
	...
	if myForm.wasSubmitted() = true then
		Call debugInfo("admin." & strContent & ".update: form was submitted, storing to the session.")
		...
		if formErrors.count = 0 then
			...
			Call debugInfo("admin." & strContent & ".update: form had zero errors")
			active = "0"
			menu = "0"
			intMenuIndex = 0
			...
			Call debug("admin." & strContent & ".update: updating database metadata content for " & strContent & "  '" & myForm.getValue(strIdField) & "'")
			dim File_Name,rs
				...
				for i = 0 to max - 1
					if len(path(i)) > 0 then
						...
						separator = "\"
							...
							Call debugInfo("Folder '" & sitepath & path_write & "' created!")
						...
						cfolder = cfolder & "../"
						Call debugInfo(sitepath & path_write & "<br>")
					end if
				next
			...
			strError = "an error was encountered during " & strContent & " data update to database"
			Call debug("admin." & strContent & ".update: inserting new " & strContent & " into database")
