...
function ShortDateFormat()
	if my_SHORT_DATE_FORMAT = 0 then
		...
		if cint(Day(testDate)) = 1 then
			...
			Call debugInfo("ShortDateFormat: day comes before month (dd/mm), eg " & Day(testDate) & " " & Monthname(Month(testDate)))
		elseif cint(Month(testDate)) = 1 then
			Call debugInfo("ShortDateFormat: month comes before day (mm/dd), eg " & Monthname(Month(testDate)) & " " & Day(testDate))
...
function formatDate(byval format,byval intTimeStamp)
	...
	if not (isnumeric(intTimeStamp)) then
		if isdate(intTimeStamp) then
			...
			Call debugError("Date.formatDate(): '" & intTimeStamp & "' is an invalid date format!")
		exit function
		end if
	end if
	...
	Call trace( "date(" & unUDate & "): Month:" & dateMonth & "(" & monthname(dateMonth) & ") Day:" & dateDay & "")
