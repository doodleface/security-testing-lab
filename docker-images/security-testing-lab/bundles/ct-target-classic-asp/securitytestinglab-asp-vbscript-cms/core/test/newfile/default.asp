...
function copy_file(byval strfilesource,byval strfiledestination,byval stroverwrite)
	...
	if strfilesource = "" or isNull(strfilesource) then
		Call debugError("sorry but a file source path is required when calling this function")
		exit function
	end if
	' error checking!!!!...
	if strfiledestination = "" or isNull(strfiledestination) then
		Call debugError("sorry but a file destination path is required when calling this function")
		exit function
	end if
	' error checking!!!!...[true - false]
	if stroverwrite = "" or isNull(stroverwrite) then
		Call debugError("parameter overwrite must be true or false")
		exit function
	end if
	if fs.fileexists(strfilesource) = false then
		Call debugError("the file does not exist...")
		exit function
	end if
	'--------------------------------[/error checking]--------------------------------
	dim f, path, max, i
	...
	for i = 0 to max - 1
		if len(path(i)) > 0 then
			...
			if fs.FolderExists(globals("SITE_PATH") & path_write) = false then
				Call debugError("Folder created!")
					set f = fs.CreateFolder(globals("SITE_PATH") & path_write)
					set f = nothing
					set fs = nothing
			else
				Call debugError("Folder exist!")
			end if
			Call debugError(globals("SITE_PATH") & path_write & "<br>")
		end if
	next
	Call debugError(globals("SITE_PATH") & path_write & "\" & path(max))
