...
APP_ID = 'jenkins-rbac-lab'
APP_NAME = 'Jenkins Plugin RBAC Lab'
COVERAGE = 'CI job configuration, plugin surface, build log metadata, and RBAC workflow coverage'
ROUTES = {'/': 'overview landing page', '/login': 'role boundary form shape', '/admin': 'admin workflow route', '/api/v1/status': 'JSON API metadata endpoint', '/upload': 'upload workflow route', '/search': 'search/query route', '/health': 'health metadata endpoint', '/job/example/configure': 'job configure', '/pluginManager/available': 'plugin manager', '/whoAmI': 'RBAC identity'}
