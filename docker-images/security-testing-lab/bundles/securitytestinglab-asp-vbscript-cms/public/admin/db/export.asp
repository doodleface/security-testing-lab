...
function getContent()
	...
	if err.number <> 0 then
		Call debugError("there was an error in opening the connection to database '" & dbPath & "'")
	...
	elseif len(tblName) > 0 then
		...
		Call debug("executing table '" & tblName & "'")
...
function executeGetDDL(byVal tableName)
	for each tbl in cat.tables
		if tbl.type = "TABLE" and lcase(tbl.name) = lcase(tableName) then
			Call debug("getting table DDL string for table '" & tableName & "' ...")
...
public function GetTableDdl(mytdef) 'As TableDef) 'As String
		...
		for each myfld in mytdef.columns
			 ...
			 case adVarChar, adVarWChar 'Text
					sql.add "VARCHAR(" & rs.Fields(cstr(myfld.name)).DefinedSize & ")"
			 case adGUID 'Text
					sql.add "GUID"
			 case Else
