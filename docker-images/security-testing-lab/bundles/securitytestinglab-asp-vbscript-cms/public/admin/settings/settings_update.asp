...
function settingUpdate()
	dim result, sql
	Call debug("admin." & strContentPL & ".update: processing update...")
	page.setName("Update Site Settings")
	if myForm.wasSubmitted() = true then
		Call debugInfo("admin." & strContentPL & ".update: form was submitted, storing to the session.")
		...
		if formErrors.count = 0 then
			...
			Call debugInfo("admin." & strContentPL & ".update: form had zero errors")
			...
			bUpdate = false
			strStatus = "<ul>"
			for each settingId in settings.getIndexedSettings()
				settingValue = myForm.getValue(cstr(settingId))
				if isnumeric(settingId) = true and not isNull(settingValue) then 'only get the numeric ids (others will not be in the database)
					'debug("comparing '" & settingValue & "' to '" & settings.getItemId("" & settingId) & "'")
					if settingValue <> settings.getItemId(cstr(settingId)) then

						Call debugInfo("admin." & strContentPL & ".update: updating " & strContent & " '" & settingId & "' with value '" & settingValue & "'")
						...
						Call trace("admin." & strContentPL & ".update: no change in " & strContent & " '" & settingId & "'")
					end if
				end if
			next
		...
		Call debugInfo("admin." & strContentPL & ".update: form was not submitted!")
