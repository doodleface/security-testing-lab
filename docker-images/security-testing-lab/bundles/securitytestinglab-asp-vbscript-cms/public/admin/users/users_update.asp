...
function getRoleLevel(strRole)
	...
	if strRole = "" or isNull(strRole) then
		Call debug("admin." & strContent & ".update: no role was provided, keeping original...")
		exit function
	elseif not isNumeric(strRole) then
		strError = "The Role '" & strRole & "'  is not a valid role level. "
		Call debugError(strError & "(must be numeric)")
		exit function
	end if
	...
	if err.number > 0 then
		strError = "The Role '" & strRole & "' is not a valid role level."
		Call debugError(strError & "(" & err.description & ")")
		exit function
	end if
