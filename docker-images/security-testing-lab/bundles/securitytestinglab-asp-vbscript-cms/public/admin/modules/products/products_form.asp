...
function buildFormContents(pk)
	...
	dim sdContent
	Call debugInfo("editing " & strContent & " with primary key='" & pk & "'")
	if pk <> "" then
	...
	if rs.state > 0 then
		...
		Call debugError("there was an error opening the following sql: " & sql)
	end if
	trapError
...
function getProductByKey(pk)
		...
		if isNull(pk) or pk = "" then
			Call debugWarning("getProductByKey(): non-null id required")
		else
			sql="SELECT * "_
				 & "FROM " & strTableName & " "_
				 & "WHERE (Key=" & pk & ");"
			...
			if rs.EOF and rs.BOF then
				Call debugError("getProductByKey(): no database content found for " & strContent & " with primary key '" & pk & "'.")
				...
				do until rs.EOF
					Call trace("getProductByKey(): the following database content found for page with pk '" & pk & "':")
					for i = 0 to counter - 1
						...
						if not sd.exists(key) then
							...
							Call debugWarning("getProductByKey(): primary key '" & pk & "' is not unique!")
							Call debugError("getProductByKey(): expected a single record but found that the recordset has multiple when field '" & rs.fields.item(i).name & "' returned a second value.")
						end if
					next
					rs.movenext
				loop
