...
function buildFormContents(pk)
	...
	dim sdContent
	Call debugInfo("editing product with primary key='" & pk & "'")
	if pk <> "" then
...
function getCategoryByKey(pk)
		...
		if isNull(pk) or pk = "" then
			Call debugWarning("getCategoryByKey(): non-null id required")
		else
			set rs = db.getRecordSet("SELECT * FROM " & strTableName & " WHERE (" & strKey & "=" & pk & ");")
			if rs.State > 0 then
				if rs.EOF and rs.BOF then
					Call debugError("getCategoryByKey(): no database content found for " & strContent & " with " & strKey & " '" & pk & "'.")
					...
					do until rs.EOF
						Call trace("getCategoryByKey(): the following database content found for " & strContent & " with " & strKey & " '" & pk & "':")
						for i = 0 to counter - 1
							key = cstr(rs.fields.item(i).name)
							val = cstr(rs(i))
							Call trace("[ " & key & " -&gt; " & Server.HtmlEncode(cstr(val)) & " ]")
							if not sd.exists(key) then
								sd.add key, val
							else
								Call debugWarning("getCategoryByKey(): " & strContent & " " & strKey & " '" & pk & "' is not unique!")
								Call debugError("getCategoryByKey(): expected a single record but found that the recordset has multiple when field '" & rs.fields.item(i).name & "' returned a second value.")
							end if
						next
						rs.movenext
					loop
