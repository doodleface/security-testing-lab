...
function contentUpdate()
	...
	if myForm.wasSubmitted() = true then
		Call debugInfo("admin.modules.update: form was submitted, storing to the session.")
		myForm.storeFormToSession()
		Call debug("admin.modules.update: done")
		set formErrors = myForm.getFormErrors()
		Call debugInfo("admin.modules.update: form had " & formErrors.count & " errors")
		if formErrors.count = 0 then
			dim intSortOrder, strPageIDs
			Call debugInfo("admin.modules.update: form had zero errors")
			strPageIDs = myForm.getValue("PageIDs")
			Call debug("admin.modules.update:  there were " & myForm.getCount("PageIDs") & " page ids selected.")
			Call debug("admin.modules.update:  the page ids string is: '" & strPageIDs & "'")
			if strPageIDs <> "" then
				strPageIDs = replace(strPageIDs, ", ", "", 1, -1, 1)
				strPageIDs = replace(strPageIDs, "::", ":", 1, -1, 1)
				Call debug("admin.modules.update:  the processed page id string is: '" & strPageIDs & "'")
			end if
			dim key, theFormDict
			...
			Call debug("admin.modules.update: updating module settings '" & myForm.getValue("PageName") & "'")
