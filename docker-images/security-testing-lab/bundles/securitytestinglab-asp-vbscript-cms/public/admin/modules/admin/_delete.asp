...
function contentDelete()
	strHeader.add "Delete " & Pcase(strContent)
	...
	Call debugInfo("admin." & strContentPL & ".delete: form was submitted, storing to the session.")
	strError = "An error was encountered during " & strContent & " deletion"
	Call debug("admin." & strContentPL & ".delete: delete database content for " & strContent & " id '" & key & "'")
	...
	if db.hasErrors() = true then
		...
		strStatus = "The " & strContent & " has been deleted. <br/>" & vbCrLf & _
				...
				"<li><a href='?create'>Upload a new " & strContent & "</a></li></ul>"
		strError = ""
		contentDelete = ""
	end if
end function
