...
if len(id)> 0 then
	Call debug("admin.mod.buildFormContents: id supplied was " & id)
	set modules = new SiteModules
	set sd = modules.getModuleById(id)
else
	Call debug("admin.mod.buildFormContents:  no id was supplied ")
...
if len(modType) > 0 then
	...
	Call debug("Modhandler is " & mods.item(cstr(modType)))
...
Call debug("admin.mod.buildFormContents: file execution returned: " & custModuleResults)
...
for each location in split(MODULE_LOCATIONS, ",")
	selected = ""
	Call debug("module location is '" & sd("Location") & "'")
...
function executeFile(byval virtualPathToModule, byval pretext)
	...
	if f.fileExists() then
		Call debug("admin.mod.executeFile: file absolute path is " & f.absolutePath)
		dim ts : set ts = fs.openTextFile(f.absolutePath, forReading)
		dim content : content = pretext & vbCrLf & replace(replace(ts.readAll, "<" & "%", ""), "%" & ">", "")
		Call debug("admin.mod.executeFile: file contents are: " & codeblock(content))
		err.clear
		on error resume next
		Execute(content)

		if err.number <> 0 then
			Call debugError("there was an error executing " & codeblock(content))
			trapError
		else
			executeFile = true
		end if
