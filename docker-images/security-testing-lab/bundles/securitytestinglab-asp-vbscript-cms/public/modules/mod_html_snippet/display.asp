...
function getContent()
	...
	Call debug("mod_html_snippet.display: starting mod_html_snippet module....")
	if settings.exists("content") then
		content = settings("content")
		Call debug("mod_html_snippet.display: content to display: " & codeblock(Server.HtmlEncode(content)))
		writeln(token_replace(content))
	else
		Call debug("mod_html_snippet.display: no content found in this snippet.")
	end if
end function
%>
