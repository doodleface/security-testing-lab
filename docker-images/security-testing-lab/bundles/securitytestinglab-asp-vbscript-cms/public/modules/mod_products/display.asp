...
function getProducts()
	...
	if len(Request.QueryString("pid")) > 0 then
		...
		Call trace("mod_products: selecting product information by PID=" & Request.QueryString("pid"))
		...
		Call trace("mod_products: ...done selecting product by PID")
		if rs.State > 0 then
			if not rs.EOF and not rs.BOF then
				Call trace("mod_products: found product '" & rs("ProductName") & "'")
				Call trace("mod_products: printing product info to page...")
				...
				Call trace("mod_products: ...end printing product")
			else
				Call debugError("mod_products: NO PRODUCT FOUND WITH PID=" & Request.QueryString("pid"))
	...
	'else show the product index page
		...
		Call trace("mod_products: selecting active products...")
		set rs = db.getRecordSet(sql)
		Call trace("mod_products: ...done selecting active products")
		Call trace("mod_products: printing product list to page...")
		...
		do while not rs.EOF and not rs.BOF
			...
			adminedit="products"
			Call debug("product_display: processing product '" & prodImgAlt & "'")
			if iscategory then
			...
			if not (isempty(rs("Image1")) = true) and (len(trim(rs("Image1"))) > 0) then
				Call debugInfo("Image1 value is '" & rs("Image1") & "'")
				Call debugInfo("Checking if image file exists at '" & prodImagePath & rs("Image1") & "'")
		...
		Call trace("mod_products: ...end printing product list, " & numProducts & " products listed.")
		numProducts = null
	end if
