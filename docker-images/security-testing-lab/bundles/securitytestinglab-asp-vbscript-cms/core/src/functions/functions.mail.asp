...
function sendMail(mail_to, mail_from, mail_subject, mail_Dict, mail_BodyHTML, mail_BodyTEXT)
	...
	if Request.ServerVariables("SERVER_NAME") <> "localhost" then
		...
		Call debugInfo("mod_form.email: SMTP HOST: " & globals("SMTPHOST"))
		flds.update
	...
	for each fldName in mail_Dict
		if lcase(fldName) = "form_name" then
			Call debugInfo("functions.mail.sendMail: Form Name is '" & Pcase(PrettyText(mail_Dict(fldName))) & "'")
		else
			displayName = Pcase(PrettyText(replace(fldName,"_"," ")))
			displayValue = Server.HtmlEncode(mail_Dict(fldName))
			Call debugInfo("functions.mail.sendMail: form item '" & displayName & "' has value '" & displayValue & "'")
			bolEven = not bolEven
			spacer = ""
			sStyle = ""
			...
			if len(displayName) < 25 then
				spacer = String(25-cint(len(displayName)), " ")
			end if
	...
	if (isNull(mail_to) or len(mail_to) = 0) then
		mail_to = """{Company Name}"" <{Contact Email}>"
	end if
	...
	if globals("BCC") = "YES" then
		oMail.BCC = token_replace("{BCCEMAIL},{ADMINEMAIL}")
	end if
	if globals("CCSENDER") = "1" then
		oMail.CC = token_replace(mail_from)
	end if
