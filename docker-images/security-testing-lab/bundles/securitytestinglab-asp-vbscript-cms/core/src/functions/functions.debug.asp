...
function debugCookies()
	if Request.Cookies().count > 0 then
		Call debug("debugCookies() { ")
		dim key
		for each key in Request.Cookies()
			Call debug("&nbsp;&nbsp;['" & key & "' -&gt; '" & Server.UrlEncode("" & Request.Cookies(key)) & "']")
		next
		Call debug("} //end debugCookies")
	else
		Call debug("debugCookies() {} //no contents")
	end if
end function
...
function debugSessionContents()
	if Session.Contents().count > 0 then
		Call debug("debugSessionContents() { ")
		dim key
		for each key in Session.Contents()
			Call debug("&nbsp;&nbsp;['" & key & "' -&gt; '" & Server.UrlEncode("" & Session.Contents(key)) & "']")
		next
		Call debug("} //end debugSessionContents ")
	else
		Call debug("debugSessionContents() {} //no contents")
	end if
end function
...
function ExecTimer(byval expression)
	...
	Execute(expression)
	t = timer - t
	Call debugInfo(left(expression, 45) & iif(len(expression) > 45, "...", "") & " took " & t & " seconds.")
	ExecTimer = t
end function
%>
