...
class SiteModules
	...
	private function initModulesDictionary()
		...
		Call trace("class.modules.init(): initializing module registry...")
		if rs.state > 0 then
			if rs.EOF and rs.BOF then
				Call debugWarning("class.modules.init(): There are no modules registered in the system.")
			else
				do while not rs.EOF
					if not sd.exists(cstr(rs("ModID"))) then
						Call trace("class.modules.init(): registering module [" & rs("ModID") & "]: '" & rs("ModName") & "' at location " & rs("ModHandler"))
						sd.add "" & rs("ModID"), "" & rs("ModHandler")
					else
						Call debugError("class.modules.init(): The key " & rs("ModID") & " already exists.")
					end if
					rs.movenext
				loop
			end if
		else
			Call debugError("class.modules.init(): the database encountered an error retrieving module types")
		end if
		if err.number <> 0 then
			Call debugError("class.modules.init: " & err.number & " - " & err.description)
			err.clear
		end if
		Call trace("class.modules.init(): initialization complete.")
	...
	public function getModAddress(itemId)
		...
		if len(itemId) > 0 then
			if myModulesDict.exists(cstr(itemId)) then
				...
				Call debug("class.modules.getItem: no item exists for the id specified '" & itemId & "'")
			end if
		end if
	...
	public function getModuleByType(byval modType)
		...
		if isNull(modType) or modType = "" then
			Call debugError("class.modules.getModuleByType: cannot execute getModuleByType('" & modType & "'). The provided module type must be non-null.")
			exit function
		end if
		...
		counter = rs.fields.count
			rs.movefirst
			do until rs.EOF
				Call trace("class.module.getModuleByType: the following database content found for module with type id '" & id & "':")
				for i = 0 to counter - 1
					...
					if not sd.exists(key) then
						...
						Call debugWarning("class.module.getModuleByType: id '" & id & "' is not unique!")
						Call debugError("class.module.getModuleByType: expected a single record but found that the recordset has multiple when field '" & rs.fields.item(i).name & "' returned a second value.")
					end if
				next
				rs.movenext
			loop
	...
	public function setPage(strPageId)
		Call debug("class.modules.setPage: '" & strPageId & "'")
		setPage = false
		...
		if rsPageModules.state > 0 then
			if rsPageModules.EOF or rsPageModules.BOF then
				Call debugInfo("class.modules.setPage: no active modules found for page with id '" & strPageId & "'")
				m_modCount = 0
			else
				setPage = true
				do until rsPageModules.EOF
					Call trace("[ Module:" & rsPageModules("ModName") & ", Location:'" & rsPageModules("Location") & "', CustomSettings:'" & Server.HtmlEncode(cstr(rsPageModules("CustomSettings"))) & "']")
			...
			Call debugError("class.modules.setPage: modules table could not be queried [state: " & rsPageModules.state & "]")
		end if
	end function
	...
	public function display(strLocation)
		...
		Call debug("class.modules.display('" & strLocation & "')...")
		if rsPageModules is nothing or isNull(rsPageModules) then
			Call debugWarning("class.modules.display: cannot execute display('" & strLocation & "').  Please initialize the module class first by specifying a page name using the setPage() method.'")
		else
			if rsPageModules.state = 0 then
				Call debug("class.modules.display: no modules to display for '" & strLocation & "'. modules recordset state is 0.")
			else
				if rsPageModules.EOF and rsPageModules.BOF then
					Call debug("class.modules.display: no modules to display for '" & strLocation & "'. modules recordset is empty.")
				else
					rsPageModules.moveFirst
					...
					do until rsPageModules.EOF
						if rsPageModules("Location") = strLocation then
							attrib.clear
							Call debugInfo("class.modules.display('" & strLocation & "': executing " & rsPageModules("ModHandler"))
							...
							Call debug("class.modules.display('" & strLocation & "'): executing '" & rsPageModules("ModHandler") & "/display.asp' with custom settings '" & Server.HtmlEncode("" & rsPageModules("CustomSettings")) & "'")
		...
		Call debug("class.modules.display('" & strLocation & "'):  " & tracker & " modules were processed.")
	end function
	...
	dim rs, sd, key, val, counter, i, sql
		set sd = Server.CreateObject("Scripting.Dictionary")
		Call debug("class.module.getModuleById:  getting page data for id '" & id & "'...")
		if isNull(id) or id = "" then
			Call debugWarning("class.module.getModuleById: non-null id required")
		else
			sql = "SELECT * FROM tblModules INNER JOIN tblModuleTypes ON tblModules.Type=tblModuleTypes.ModID WHERE tblModules.ID=" & id
			set rs = db.getRecordSet(sql)
			if rs.state > 0 then
				if rs.EOF and rs.BOF then
					Call debugError("class.module.getModuleById: no database content found for module with id '" & id & "'.")
					...
					counter = rs.fields.count
					do until rs.EOF
						Call trace("class.module.getModuleById: the following database content found for module with id '" & id & "':")
						for i = 0 to counter - 1
							key = cstr(rs.fields.item(i).name)
							val = cstr(rs(i))
							Call trace("[ " & key & " - & gt; " & Server.HtmlEncode(val) & " ]")
							if not sd.exists(key) then
								...
								Call debugWarning("class.module.getModuleById: id '" & id & "' is not unique!")
								Call debugError("class.module.getModuleById: expected a single record but found that the recordset has multiple when field '" & rs.fields.item(i).name & "' returned a second value.")
							end if
						next
						rs.movenext
					loop
	...
	public sub executeMod(strHandler, strCustomSettings)
		Call debug("class.modules.executeMod: executing module '" & strHandler & "'")
		Session("ModuleCustomSettings") = ""
		if isNull(strCustomSettings) or strCustomSettings = "" then
			Call debug("class.modules.executeMod: no custom settings for this module")
		else
			Call debug("class.modules.executeMod: adding custom setting string to session '" & Server.HtmlEncode(strCustomSettings) & "'")
		...
		Call debug("class.modules.executeMod: executing '" & modFile.VirtualPath & "' ...")
		modFile.run()
		'session("ModuleCustomSettings") = ""
	end sub
end class
