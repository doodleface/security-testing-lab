...
class AspPage
	...
	private function initialize(filepath)
		Call debugInfo("initializing file at path " & filepath)
		m_exists = false
		initialize = false
		...
		if isNull(m_filepath) or m_filepath = "" then
			Call debugWarning("class.page.init: filepath must be non-null")
		else
			...
			Call debug("class.page.init: page data dict size " & m_info.count)
			id = m_info.item("PageID")
			if m_info.count = 0 or id = "" then
				Call debugWarning("class.page.init: no id found for filepath '" & m_filepath & "'")
				initialize = true
			else
				Call trace("class.page.init: obtaining page data for id '" & id & "'")
				m_hasModules = m_modules.setPage(id)
				if m_hasModules = true then
					Call debugInfo("class.page.init: page has " & m_modules.count & " custom m_modules to display")
				else
					Call debugInfo("class.page.init: page has NO custom m_modules to display!")
				end if
	...
	public function setFile(filename)
		dim result
		Call debug("class.page.setFile: '" & filename & "'")
		if isNull(filename) or filename = "" then
			Call debugWarning("class.page.setFile: non-null filename required")
			result = false
		else
			result = initialize(filename)
			if result = true then
				Call trace("class.page.setFile: page initialized successfully")
			else
				Call debugError("class.page.setFile: There was an error initializing the page")
			end if
		end if
		setFile = result
	end function
	...
	public function setCharEncoding(strCharSet)
		Call trace("class.page.setCharEncoding: '" & strCharSet & "'")
	...
	public function addBreadcrumb(strName, strURL)
		Call debugInfo("adding BREADCRUMB: '" & strName & "' url '" & strURL & "'")
		dim lastCrumb : lastCrumb = ubound(m_breadcrumbs, 2)
		Call debugInfo("number of crumbs is  '" & lastCrumb & "'")
	...
	public function getBreadcrumbs()
		...
		for i=0 to ubound(m_breadcrumbs, 2)
			...
			title = "Open " & name
			Call debugInfo("adding breadcrumb '" & i & "'")
			Call debugInfo("... url '" & url & "', name '" & name & "'")
	...
	public function setStyle(byval strStyleSheetName)
		...
		if fileExists("/styles/" & strStyleSheetName & ".css") then
			Call trace("class.page.setStyle:  style set to '" & strStyleSheetName & "'")
	...
	public function overrideStyle(byval strStyleSheetName)
		overrideStyle = FALSE
		Call trace("class.page.overrideStyle:  attempting to override stylesheet with '" & strStyleSheetName & "'")
	...
	public function getRandomChildPageContent(strFilePath)
		Call debugError("class.page.getRandomChildPageContent:  this method is deprecated, please use module mod_random_content instead!")
	end function
	...
	public function hasParent()
		...
		Call trace("class.page.hasParent: " & result)
		hasParent = result
	end function
	...
	public function hasContent()
		...
		hasContent = result
		Call trace("class.page.hasContent: " & result)
	end function

	public function exists()
		exists = m_exists
		Call trace("class.page.exists: " & m_exists)
	end function
	...
	public function isAdminPage()
		...
		Call trace("class.page.isAdminPage: getfilepath=" & getFilePath)
		Call trace("class.page.isAdminPage: adminDir=" & globals("ADMIN_DIR"))
		Call trace("class.page.isAdminPage: " & result)
		isAdminPage = result
	end function
	...
	public function getPageDataById(pageID)
		...
		Call debug("class.page.getPageDataById:  getting page data for pageID '" & pageID & "'...")
		if isNull(pageID) or pageID = "" then
			Call debugWarning("class.page.getPageDataById: non-null pageID required")
		else
			...
			if rs.state > 0 then
				if rs.EOF and rs.BOF then
					Call debugError("class.page.getPageDataById: no database content found for page with id '" & id & "'.")
					...
					do until rs.EOF
						Call trace("class.page.getPageDataById: the following database content found for page with id '" & id & "':")
						for i=0 to counter-1
							key = "" & rs.fields.item(i).name
							val = "" & rs(i)
							Call trace("[ " & key & " - & gt; " & Server.HtmlEncode("" & val) & " ]")
							if not sd.exists(key) then
								sd.add key, val
							else
								Call debugWarning("class.page.getPageDataById: id '" & id & "' is not unique!")
								Call debugError("class.page.getPageDataById: expected a single record but found that the recordset has multiple when field '" & rs.fields.item(i).name & "' returned a second value.")
							end if
						next
						rs.movenext
					loop
	...
	public function getHomepageData()
		dim rs, sd, key, val, counter, i, sql
		set sd = Server.CreateObject("Scripting.Dictionary")
		Call debug("class.page.getHomepageData:  getting homepage data...")
		sql = "SELECT TOP 1 tblPages.*, tblPageContent.ContentID, tblPageContent.PageContent, tblPageContent.ModifiedDate "_
			& "FROM tblPages "_
		...
		if rs is nothing then
			Call debugError("class.page.getHomepageData: database could not be found.")
		elseif rs.state > 0 then
			if rs.EOF and rs.BOF then
				Call debugError("class.page.getHomepageData: no homepage could be found in the database.")
			else
				rs.movefirst
				counter = rs.fields.count
				do until rs.EOF
					Call trace("class.page.getHomepageData: the following homepage content was found:")
					for i=0 to counter-1
						key = "" & rs.fields.item(i).name
						val = "" & rs(i)
						Call trace("[ " & key & " - & gt; " & Server.HtmlEncode("" & val) & " ]")
						if not sd.exists(key) then
							...
							Call debugError("class.page.getHomepageData: expected a single record but found that the recordset has multiple when field '" & rs.fields.item(i).name & "' returned a second value.")
						end if
					next
					rs.movenext
				loop
	...
	public function getPageDataByFileName(strFilePath)
		dim rs, sd, key, val, counter, i, sql
		set sd = Server.CreateObject("Scripting.Dictionary")
		Call debug("class.page.getPageDataByFileName:  getting page data for path '" & strFilePath & "'...")
		if isNull(strFilePath) or strFilePath = "" then
			Call debugWarning("class.page.getPageDataByFileName: non-null path required")
		else
			sql = "SELECT TOP 1 tblPages.*, tblPageContent.ContentID, tblPageContent.PageContent, tblPageContent.ModifiedDate "_
			& "FROM tblPages LEFT JOIN tblPageContent ON tblPages.PageID = tblPageContent.PageID "_
			...
			if (rs is nothing) then
				Call debugError("class.page.getPageDataByFileName: database returned no result for query: " & sql)
			elseif rs.state > 0 then
				if rs.EOF and rs.BOF then
					Call debugError("class.page.getPageDataByFileName: no database content found for page with path '" & strFilePath & "'.")
					...
					counter = rs.fields.count
					do until rs.EOF
						Call trace("class.page.getPageDataByFileName: the following database content found for page with path '" & strFilePath & "':")
						for i = 0 to counter - 1
							key = "" & rs.fields.item(i).name
							val = "" & rs(i)
							Call trace("[ " & key & " - & gt; " & Server.HtmlEncode("" & val) & " ]")
							if not sd.exists(key) then
								...
								Call debugWarning("class.page.getPageDataByFileName: filePath '" & strFilePath & "' is not unique!")
								Call debugError("class.page.getPageDataByFileName: expected a single record but found that the recordset has multiple when field '" & rs.fields.item(i).name & "' returned a second value.")
							end if
						next
						rs.movenext
					loop
	...
	private function getPageIdByName(strName)
		...
		if isNull(strName) or strName = "" then
			Call debugWarning("class.page.getPageIdByName: non-null name required")
		else
			...
			set rs = db.getRecordSet(sql)
			if rs.EOF and rs.BOF then
				Call debugWarning("class.page.getPageIdByName: no id for page with name '" & strName & "'.")
				getPageIdByName = ""
			else
				Call debug("class.page.getPageIdByName: relative file path '" & strFilePath & "' found with id '" & rs(0) & "'.")
	...
	public function getPageIdByFilePath(strFilePath)
		Call debugWarning("page.class.getPageIdByFilePath: this method is deprecated,  please create a new page and use the getId() method")
		'dim rs
		'if isNull(strFilePath) or strFilePath = "" then
	...
	public function display(strLocation)
		...
		Call trace("class.page.display('" & strLocation & "') ...")
		...
		Call trace("...end class.page.display('" & strLocation & "')")
	end function
  ...
  ' return a string of the html formatted link structure
	...
	public function getLocalLinks()
		...
		if not exists then
			Call debugWarning("class.page.getLocalLinks: no local links to get for non-existent page")
		else
			if hasParent then
				'get siblings having same parent page id
				id = getParentPageId
				strHeading = getParentPageName
				Call debug("class.page.getLocalLinks: obtaining siblings of " & getName & " also having ParentPage id '" & id & "'")
			else
				'get children that have my id as a parent page
				id = getId
				strHeading = getName
				Call debug("class.page.getLocalLinks: obtaining children of " & strHeading & " with id '" & id & "'")
			end if
			...
			if rsLocalPages.state > 0 then
				if rsLocalPages.EOF and rsLocalPages.BOF then
					Call debug("class.page.getLocalLinks: there are no children or siblings to list for this page")
				else
