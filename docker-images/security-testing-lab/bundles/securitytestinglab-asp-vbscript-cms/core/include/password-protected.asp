...
'* dim PASSWORD : PASSWORD = "[REDACTED-SAMPLE]"
'* %&gt;
...
function customContent(section)
	...
	if err.number <> 0 then
		Call debugError("password-protected.asp: you must set the password before using the password-protected.asp script!")
	...
	with the_form
		...
		if (.wasSubmitted = true and .hasErrors = false) then
			...
			Call debug("password-protected.asp: user provided password: " &user_supplied_password)
			session("pass") = user_supplied_password
		end if
		if user_supplied_password = PASSWORD then
			Call debug("password-protected.asp: the password is valid!")
			exit function
		end if
		Call debug("password-protected.asp: the passowrd '" & user_supplied_password & "' is invalid!")
