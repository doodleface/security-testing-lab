...
function getTestimonialById(id)
	...
	if rs.state > 0 then
		if rs.EOF and rs.BOF then
			...
			Call debugError("no data found for page with id '" & id & "'.")
		else
			rs.movefirst
			counter = rs.fields.count
			do until rs.EOF
				Call trace("the following database content found for testimonial with id '" & id & "':")
				for i = 0 to counter - 1
					...
					if not sd.exists(key) then
						...
						Call debugError("expected result set of 1 item but got more!  A value for '" & rs.fields.item(i).name & "' already exists!")
					end if
				next
				trapError
				rs.movenext
