...
function contentAdd()
	...
	if myForm.wasSubmitted() = false then
		...
		Call debugError("admin.filemanager.add: " & strError)
		exit function
	end if
	...
	Call debugInfo("admin.filemanager.add: form was submitted, storing to the session.")
	myForm.storeFormToSession()

	Call debugInfo("admin.filemanager.add: form had zero errors")
	sFileName = myForm.getValue("File")
	sPath = myForm.getValue("Dir")

	writeln( p("Path is '" & sPath & "'"))

	Call debug("admin.filemanager.add: done")
	set formErrors = myForm.getFormErrors()
	Call debugInfo("admin.filemanager.add: form had " & formErrors.count & " errors")
	...
	Call debug("admin.filemanager.add: setting upload path to '" & spath & "'")
	myForm.uploadPath = sPath


	Call debug("admin.filemanager.add: checking existence of file...")
	...
	Call debug("admin.filemanager.add: sending file up...")
	...
	Call debug("admin.filemanager.add: file up complete.")
