...
function contentAdd()
	...
	if myForm.wasSubmitted() = true then
		Call debugInfo("admin.modules.add: form was submitted, storing to the session.")
		myForm.storeFormToSession()
		Call debug("admin.modules.add: done")
		set formErrors = myForm.getFormErrors()
		Call debugInfo("admin.modules.add: form had " & formErrors.count & " errors")
		if formErrors.count = 0 then
			dim intSortOrder, strPageIDs
			Call debugInfo("admin.modules.add: form had zero errors")
			strPageIDs = myForm.getValue("PageIDs")
			Call debug("admin.modules.add:  there were " & myForm.getCount("PageIDs") & " page ids selected.")
			Call debug("admin.modules.add:  the page ids string is: '" & strPageIDs & "'")
			if strPageIDs <> "" then
				strPageIDs = replace(strPageIDs,", ","",1,-1,1)
				strPageIDs = replace(strPageIDs,"::",":",1,-1,1)
				Call debug("admin.modules.add:  the processed page id string is: '" & strPageIDs & "'")
			end if
			dim key, theFormDict
			...
			Call debug("admin.modules.add: updating module settings '" & myForm.getValue("PageName") & "'")
