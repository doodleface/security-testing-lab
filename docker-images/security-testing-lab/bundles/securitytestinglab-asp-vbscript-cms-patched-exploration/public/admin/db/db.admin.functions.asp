...
public function CreateSQL(byval queryType, byref strTableName,byref objContentDict,byval strPKid)
  ...
  next
	...
	for each myfld in tbl.columns
		...
		Call debug("db.admin.functions.CreateSQL: field " & myfld & " has value " & sqlValue)
...
public function createDB(ByVal filePath)
	...
	dim result
	Call debug("does " & myPath & " exist? " & fs.fileExists(myPath))
	if fs.fileExists(myPath) = true then
		result = true
	elseif fs.folderExists(myParent) = true then
		Call debug("creating database file " & myPath)
		cat.create dbCreate
		if err.number <> 0 then
			Call debugError("there was an error in database creation at filepath (" & myPath & ")")
			Call debugError("error in " & Err.source & " error code " & err.number & ": " & err.description)
			err.clear
		end if
		result = true
	else
		Call debug("parent folder doesnt exist,  building parent folder " & myParent)
		call buildDirectories(myParent, 1)
		result = true
	end if
	Call debug("result is " & result & " for " & myPath)
end function
...
public sub buildDirectories(ByVal folderPath, ByRef counter)
	...
	Call debug("" & counter & " does " & i & " exist? " & fs.folderExists(i))
	Call debug("" & counter & " does " & p & " exist? " & fs.folderExists(p))
	if not fs.folderExists(p) then
		call buildDirectories(p, counter + 1)
	end if
	Call debug("creating " & i)
	fs.createFolder(i)
	if err.number <> 0 then
		Call debugError("there was an error in recursion at filepath (" & i & ")")
		if err.number = 70 then debugError("the IUSR_ doesnt have write permissions in folder " & p)
		Call debugError("error was " & err.number & ": " & err.description)
		err.clear
	end if
	i = null
end sub
