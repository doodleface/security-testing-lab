...
function CreateDictionary(objDict,strList,strItemDelimiter,strKeyValDelimiter,collisionResolution)
	...
	if not isObject(objDict) then
		Call trace("functions.lists.createDictionary: objDict provided is not an object, creating new dictionary")
		set objDict = nothing 'clear pre existing object contents
		set objDict = Server.CreateObject("Scripting.Dictionary")
	else
		Call trace("functions.lists.createDictionary: inserting items into dictionary...")
	end if
	...
	for i = 0 to ubound(propsArray)
		if instr(propsArray(i), "" & strKeyValDelimiter) > 0 then
			...
			if err.number <> 0 then
				Call debugError("functions.lists.createDictionary: an error occurred durring key/value separation for string '" & Server.HtmlEncode(propsArray(i)) & "'")
				Call debugError("functions.lists.createDictionary: key is '" & key & "'")
				Call debugError("functions.lists.createDictionary: value is '" & val & "'")
				Call debugError("functions.lists.createDictionary: Key/Value Delimiter is '" & strKeyValDelimiter & "'")
				Call debugError("functions.lists.createDictionary: Record Delimiter is '" & strItemDelimiter & "'")
				trapError
				on error goto 0
			else
				...
				Call trace(" [ " & key & " -> " & Server.HtmlEncode("" & objDict.item(key)) & "]")
			end if
		end if
	next
end function
