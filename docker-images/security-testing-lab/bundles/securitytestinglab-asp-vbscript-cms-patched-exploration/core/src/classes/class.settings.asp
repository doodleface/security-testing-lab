...
class SiteSettings
	...
	private sub initializeSiteSettings()
		...
		err.clear
		if rs.state = 0 then
			Call debugError("unable to process site settings, the database tblGlobalSettings was not found.")
		elseif rs.EOF or rs.BOF then
			Call debugError("The database table tblGlobalSettings was empty!")
		else
			Call trace("class.settings.init:  SETTINGS:")
			rs.movefirst
			do until rs.eof
				...
				if isEmpty(val) or val = "" then
					Call trace(" [ " & key & " -> UNDEFINED ]")
				else
					Call trace(" [ " & key & " -> " & Server.HtmlEncode(val) & " ]")
				end if
				if not sdNamedSettings.exists(key) then
					...
					Call debugError("class.settings.init: expected '" & key & "' to be a unique setting but encountered a second.")
				end if
				trapError
				rs.movenext
			loop
		...
		if instr(str, TOKEN_PREFIX) > 0 then
			...
			if matched.count > 0 then
				for each expr in matched
					...
					Call trace("class.settings.token_replace: '" & variableName & "'")
...
function token_decode(byval plaintext, byval variables)
	...
	Call trace("class.settings.token_decode: '" & variables & "'")
	variables = split(variables, ",")
	Call trace("class.settings.token_decode: list has " & ubound(variables) & "  items")
	...
	dim i : i = 0
	do
		Call trace("class.settings.token_decode: decoding '" & variables(i) & "'")
		Call trace("class.settings.token_decode: replaceing '" & variables(i) & "' with '" & globals(cstr(variables(i))) & "'")
...
function token_encode(byval plaintext, byval variables)
	...
	Call trace("class.settings.token_encode: '" & variables & "'")
	variables = split(variables, ",")
	Call trace("class.settings.token_encode: list has " & ubound(variables) & "  items")
	plaintext = replace(plaintext, Server.UrlEncode(TOKEN_PREFIX), TOKEN_PREFIX)
	plaintext = replace(plaintext, Server.UrlEncode(TOKEN_SUFFIX), TOKEN_SUFFIX)
	dim i
