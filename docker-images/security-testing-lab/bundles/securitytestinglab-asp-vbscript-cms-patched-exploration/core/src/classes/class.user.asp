...
class ClientUser
	...
	private sub class_Initialize()
		...
		if session(USER_COOKIE) <> "" then
			...
			bolExists = true
			Call debugInfo("class.user.init: aknowledged active user session")
		elseif Request.Cookies(USER_COOKIE) <> "" then
			myUserDataString = decrypt(Request.Cookies(USER_COOKIE))
			bolExists = true
			Call debugInfo("class.user.init: aknowledged user from cookies")
			rememberMe(true)
		else
			myUserDataString = DEFAULT_USER_STRING
			bolExists = false
			Call debug("class.user.init: No user data was found for this client session, using defaults.")
			rememberMe(DEFAULT_PERSISTENCE_POLICY)
		end if
		Call trace("class.user.init: un-encrypted user data '" & myUserDataString & "'")
	...
	public function setId(userId)
		Call trace("class.user.setId: '" & userId & "'")
		myUserId = userId
	end function
	...
	public function setName(userName)
		Call trace("class.user.setName: '" & userName & "'")
		myUserName = userName
	end function
	...
	public function setFullName(userNameFull)
		Call trace("class.user.setFullName: '" & userNameFull & "'")
	...
	Call trace("class.user.isAdministrator: '" & (getRole()>=50) & "'")
		isAdministrator = (getRole()>=50)
	end function

	public function setAdmin(isApproved)
		Call trace("class.user.setAdmin:'" & isApproved & "'")
		Call debugWarning("class.user.setAdmin: THIS METHOD Has been deprecated.  you should not ever have to setAdmin(), use setRole(intValue) instead.")
		hasAdminRights = cbool(isApproved)
	end function

	public function setRole(byVal roleLevel)
		on error resume next
		Call trace("class.user.setRole: '" & roleLevel & "'")
		...
		if (err.number <> 0) or (myRoleLevel < 0) then
			Call debugError("class.user.setRole: bad role '" & roleLevel & "'.  Must specify an integer greater than 0.")
		end if
	end function
	...
	public function isLoggedIn()
		Call trace("class.user.isLoggedIn: session logged-in date: " & cdate(decrypt(session(getId()))))
		if cdate(decrypt(session(getId()))) = date() then
			Call debugInfo("class.user.isLoggedIn: User '" & myUserId & "' is logged in.")
			isLoggedIn = true
		else
			Call debugInfo("class.user.isLoggedIn: User '" & myUserId & "' is not logged in.")
			isLoggedIn = false
		end if
	end function

	public function setLoggedIn(bool)
		Call trace("class.user.setLoggedIn: '" & cbool(bool) & "'")
		if getId() <> "" then
			if cbool(bool) = true then
				session("" & getId()) = encrypt("" & date())
			else
				Call trace("setting session cookie to " & date() - 91)
	...
	public function login(userId, password)
		...
		setId(userId)
		if isLoggedIn() then
			Call debugWarning("class.user.login: user '" & userId & "' is already logged in")
			loginstatus = true
		else
			Call trace("class.user.login: Logging in as user '" & userId & "'...")
			'Execute the verification process
			set rs = db.getRecordSet("SELECT * FROM tblUsers WHERE UserID='" & userId & "'")

			if rs.state > 0 then
				'1 - Password check
				Call debug("class.user.login: userid=" & userId)
				Call debug("class.user.login: password=" & password)
				...
				elseif rs("Disabled")  then
					...
					Call debug("class.user.login: disabled=" & rs("Disabled"))
					Call debug("class.user.login: firstname=" & rs("FirstName"))
				end if
			...
			if err.number <> 0 then
				Call debugWarning("class.user.login(): Trapped an error during DB record set processing")
				processErrors
				loginstatus = false
			end if
		end if
	...
	public function persist(expirationTime)
		...
		Call trace("Persisting user data for User '" & getName() & "'...")
		Call trace("...data string: '" & a & "'")
		session(USER_COOKIE)= encrypt(a)
		Call trace("...encrypted session cookies: '" & Server.UrlEncode("" & session(USER_COOKIE)) & "'")
		if (isPersistent = true) then
			Response.Cookies(USER_COOKIE) = session(USER_COOKIE)
			Call trace("...encrypted client cookies: '" & Server.UrlEncode("" & session(USER_COOKIE)) & "'")
			Response.Cookies(USER_COOKIE).expires = expirationTime
			Call trace("...the cookie '" & USER_COOKIE & "' expires '" & expirationTime & "'")
		else
			unpersist()
		end if
	end function
	...
	public function unpersist()
		Call trace("User requested to not retain Cookies...")
		Call debug("... the cookie '" & USER_COOKIE & "' set to expire on '" & Date() - 1 & "'")
	...
	private function addError(strErrorMessage)
		Call debugError("class.user.Error: " & Server.HtmlEncode(strErrorMessage))
